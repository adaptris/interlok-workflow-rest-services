 ...
 *
 * <p>
 * The use case for this is something like Heroku which generally only allows you to expose a single external port. If you have an existing
 * API endpoint listening via <i>embedded jetty</i> and you have also require <i>jolokia</i> then you need to jump through additional hoops
 * to make this possible (it could be done by having an additional API endpoint workflow that proxies everything to the jolokia endpoint).
 * This is a very simplified proxy simply passes through all headers as-is to the proxied URL.
 * </p>
 *
 * <p>
 ...
 * </pre>
 *
 * In this case we would register as a listener against the URI <i>/jolokia/*</i> and <i>/alternate/*</i> against the embedded jetty
 * component. All traffic to our registered URI (e.g. http://localhost:8080/jolokia/my/jolokia/action) would be forwarded to the
 * corresponding server/port combination (http://localhost:8081/jolokia/my/jolokia/action).
 * </p>
 */
