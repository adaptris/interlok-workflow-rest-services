...
public class WorkflowServicesComponentTest {
  ...
  @Mock
  private WorkflowServicesConsumer mockConsumer;

  @Mock
  private InterlokJmxClient mockJmxClient;

  @Mock
  private MBeanServer mockMbeanServer;

  @Mock
  private AdaptrisMessageFactory mockMessagefactory;
  ...
  public void testProcessingWorkflowsDefinition() throws Exception {
    ...
    when(mockMessagefactory.newMessage()).thenReturn(returnedMessage);

    when(mockMbeanServer.queryMBeans(any(), any())).thenReturn(mockReturnedWorkflows);
  ...
  public void testInitFails() throws Exception {
    doThrow(new CoreException("Expected")).when(mockConsumer).init();
